...
class read_data():
    def __init__(self, opt):
        ...
        real_img_dir = os.path.join(self.root, '0_real')
        fake_img_dir = os.path.join(self.root, '1_fake')

        if not os.path.exists(real_img_dir) or not os.path.exists(fake_img_dir):
            raise FileNotFoundError("Specified directory does not exist: {}".format(self.root))

        real_img_list = [os.path.join(real_img_dir, train_file) for train_file in os.listdir(real_img_dir)]
        real_label_list = [0 for _ in range(len(real_img_list))]

        fake_img_list = [os.path.join(fake_img_dir, train_file) for train_file in os.listdir(fake_img_dir)]
